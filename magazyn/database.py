...
def init_db():
    ...
    with get_conn() as conn:
        ...
        indices = [
            ...
            "CREATE UNIQUE INDEX IF NOT EXISTS uq_app_users_login_ci ON app_users(LOWER(login))",
        ]
...
def seed_auth_defaults() -> None:
    ...
    with get_conn() as conn:
        ...
        if int(cur.fetchone()[0]) == 0:
            cur.execute(
                "INSERT INTO app_users(login, password_hash, role_id, is_active, created_at) VALUES (?, NULL, ?, 1, ?)",
                ("admin", admin_role_id, now),
            )
            log.warning("Brak skonfigurowanego hasła administratora. Ustaw hasło dla konta 'admin' przy pierwszym logowaniu.")
        conn.commit()
...
def authenticate_user(login: str, password: str):
    with get_conn() as conn:
        ...
        if int(row[5]) != 1:
            return None
        packed = str(row[2] or "").strip()
        if not packed:
            return None
        if not _verify_password(password, packed):
            return None
        return {
            "id": int(row[0]),
            "login": str(row[1]),
...
def is_password_setup_required(login: str) -> bool:
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute("SELECT password_hash FROM app_users WHERE LOWER(login)=LOWER(?)", ((login or "").strip(),))
        row = cur.fetchone()
        if not row:
            return False
        return not str(row[0] or "").strip()


def set_user_password_by_login(login: str, new_password: str) -> bool:
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            "UPDATE app_users SET password_hash=?, updated_at=? WHERE LOWER(login)=LOWER(?)",
            (_password_hash(new_password), now, (login or "").strip()),
        )
        conn.commit()
...
def create_user(login: str, password: str, role_id: int) -> None:
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            (normalized_login, _password_hash(password), int(role_id), now),
        )
        conn.commit()
...
def migrate_db():
    ...
    with get_conn() as conn:
        ...
        cur.execute("UPDATE devices SET received_date='1970-01-01' WHERE received_date IS NULL OR received_date='' OR received_date NOT GLOB '????-??-??'")
        ...
        cur.execute("UPDATE deliveries SET delivery_date='1970-01-01' WHERE delivery_date IS NULL OR delivery_date='' OR delivery_date NOT GLOB '????-??-??'")
        cur.execute("UPDATE deliveries SET invoice_vat = CASE WHEN invoice_vat IN (1, '1', 'true', 'TRUE') THEN 1 ELSE 0 END")
        ...
        cur.execute("UPDATE app_users SET login = LOWER(TRIM(login)) WHERE login IS NOT NULL")

        # Rebuild tables with stronger constraints.
        cur.execute("PRAGMA foreign_keys=OFF")

        cur.execute("""
            CREATE TABLE IF NOT EXISTS devices_new (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                received_date TEXT NOT NULL CHECK(received_date GLOB '????-??-??'),
        ...
        """)
        cur.execute("DELETE FROM devices_new")
        cur.execute("""
            INSERT INTO devices_new(id, received_date, item_type, device_name, serial_number, imei1, imei2,
                                    production_code, notes, delivery_id, created_at, updated_at)
            SELECT id,
                   CASE
                     WHEN received_date IS NULL OR received_date='' OR received_date NOT GLOB '????-??-??' THEN '1970-01-01'
                     ELSE received_date
                   END,
                   CASE WHEN item_type IN ('device','accessory') THEN item_type ELSE 'device' END,
                   device_name, serial_number, imei1, imei2,
                   production_code, notes, delivery_id,
                   COALESCE(created_at, datetime('now')),
                   updated_at
            FROM devices
        """)
        cur.execute("DROP TABLE devices")
        cur.execute("ALTER TABLE devices_new RENAME TO devices")

        cur.execute("""
            CREATE TABLE IF NOT EXISTS deliveries_new (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                delivery_date TEXT NOT NULL CHECK(delivery_date GLOB '????-??-??'),
                sender_name TEXT,
                courier_name TEXT,
                delivery_type TEXT NOT NULL,
                tracking_number TEXT,
        ...
        """)
        cur.execute("DELETE FROM deliveries_new")
        cur.execute("""
            INSERT INTO deliveries_new(id, delivery_date, sender_name, courier_name, delivery_type,
                                       tracking_number, invoice_vat, notes, created_at, updated_at)
            SELECT id,
                   CASE
                     WHEN delivery_date IS NULL OR delivery_date='' OR delivery_date NOT GLOB '????-??-??' THEN '1970-01-01'
                     ELSE delivery_date
                   END,
                   sender_name,
                   courier_name,
                   COALESCE(delivery_type, 'MAGAZYN'),
                   tracking_number,
                   CASE WHEN invoice_vat IN (1, '1', 'true', 'TRUE') THEN 1 ELSE 0 END,
                   notes,
                   COALESCE(created_at, datetime('now')),
                   updated_at
            FROM deliveries
        ...
        """)
        cur.execute("DELETE FROM app_users_new")
        cur.execute("""
            INSERT INTO app_users_new(id, login, password_hash, role_id, is_active, recovery_email,
                                      reset_code_hash, reset_code_expires, created_at, updated_at)
            SELECT id,
                   LOWER(TRIM(login)),
                   NULLIF(TRIM(COALESCE(password_hash,'')), ''),
                   role_id,
        ...
        cur.execute("ALTER TABLE app_users_new RENAME TO app_users")

        cur.execute("PRAGMA foreign_keys=ON")

        # recreate indexes after table rebuild
        cur.execute("CREATE INDEX IF NOT EXISTS idx_devices_received_date ON devices(received_date)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_devices_serial_number ON devices(serial_number)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_devices_imei1 ON devices(imei1)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_devices_imei2 ON devices(imei2)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_devices_item_type ON devices(item_type)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_devices_delivery_id ON devices(delivery_id)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_deliveries_date ON deliveries(delivery_date)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_deliveries_type ON deliveries(delivery_type)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_deliveries_sender ON deliveries(sender_name)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_delivery_attachments_delivery ON delivery_attachments(delivery_id)")
        cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_app_users_login_ci ON app_users(LOWER(login))")
