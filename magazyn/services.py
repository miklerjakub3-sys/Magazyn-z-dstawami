...
class MagazynService:
    ...
    def is_password_setup_required(self, login: str) -> bool:
        return database.is_password_setup_required(login)

    def setup_password_for_login(self, login: str, new_password: str) -> None:
        if len((new_password or "").strip()) < 8:
            raise ValueError("Hasło musi mieć co najmniej 8 znaków.")
        ok = database.set_user_password_by_login(login, new_password)
        if not ok:
