...
class SettingsPage(QWidget):
    ...
    def restore_selected_backup(self) -> None:
        ...
        entered_password = password.text().strip()
        if not entered_password and BACKUP_ZIP_PASSWORD:
            entered_password = BACKUP_ZIP_PASSWORD
        try:
    ...
    def add_user(self) -> None:
        ...
        if d.exec() != QDialog.DialogCode.Accepted:
            return
        _admin_password, login, password, role_id = d.get_data()
    ...
    def save_selected_user_role_and_permissions(self) -> None:
        ...
        if not self.svc.has_permission("users.manage"):
